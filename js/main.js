...
function getGenerators(){
    ...
    let gens = [];
    let gen = ' ';
    for(i = 0; i < lattice1.generators.length; i++){
        let set = '{';
        if (lattice1.generators.length == 0){
            set = '{}';
        } else {
            for(j=0; j < lattice1.generators[i].length; j++){
    ...
    gens.push(gen)
    gen = ''
    for(i = 0; i < lattice2.generators.length; i++){
        let set = '{';
        if (lattice2.generators.length == 0){
            set = '{}';
        } else {
            for(j=0; j < lattice2.generators[i].length; j++){
...
function getSetString(d){
    ...
    for(i = 0; i < set_indices.length; i++){
...
function initializeMap(d){
    ...
    parent_id = d3.select(this.parentNode).attr('id');
    let indices;
    let label;
    if (parent_id == 'maps1'){
